...
# Dependent services
POST_SERVICE_HOST = ENV.fetch("POST_SERVICE_HOST") { "127.0.0.1" }
POST_SERVICE_PORT = ENV.fetch("POST_SERVICE_PORT") { 4567 }
COMMENT_SERVICE_HOST = ENV.fetch("COMMENT_SERVICE_HOST") { "127.0.0.1" }
COMMENT_SERVICE_PORT = ENV.fetch("COMMENT_SERVICE_PORT") { 4567 }
ZIPKIN_ENABLED = ENV.fetch("ZIPKIN_ENABLED") { false }
POST_URL = "http://#{POST_SERVICE_HOST}:#{POST_SERVICE_PORT}".freeze
COMMENT_URL = "http://#{COMMENT_SERVICE_HOST}:#{COMMENT_SERVICE_PORT}".freeze

# App version and build info
VERSION = File.exist?("VERSION") ? File.read("VERSION").strip : "version_missing"
BUILD_INFO = File.exist?("build_info.txt") ? File.readlines("build_info.txt") : Array.new(2, "build_info_missing")
...
use ZipkinTracer::RackHandler, zipkin_config if settings.zipkin_enabled?
